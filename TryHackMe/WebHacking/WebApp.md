...
Request Headers: Host, User-Agent, Referer, Cookie, Content-Type  
Request Bodies: x-www-form-urlencoded, form-data, json, xml  
...
Status Codes: Informational 1XX, Successful 2XX, Redirection Messages 3XX, Client Error 4XX, Server Error 5XX  
...
Which HTTP status code indicates that the requested resource could not be found on the web server? -404  1)  
start
...
9) Security Headers  
Content-Security-Policy (CSP): Content-Security-Policy: default-src 'self'; script-src 'self' https://cdn.tryhackme.com; style-src 'self'  
...
GET to /api/users -THM{YOU_HAVE_JUST_FOUND_THE_USER_LIST}  
POST to /api/user/2 and values name=Bob&&country=US -THM{YOU_HAVE_MODIFIED_THE_USER_DATA}  
DELETE to /api/user/1 -THM{YOU_HAVE_JUST_DELETED_A_USER}
